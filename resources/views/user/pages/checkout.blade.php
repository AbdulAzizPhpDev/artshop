...
@section('content')
    <div class="checkout" style="margin-bottom: 50px">
        <div class="content">
            ...
            <form id="form_id" action="{{route('user.order.create',app()->getLocale())}}" method="post" class="b1">
                @csrf
                <div class="b2">
                    @if(!auth()->check())
                        ...
                        <div class="b4">
                            <div class="b5">
                                ...
                                <a style="display: block;width: 177px!important;" href="{{route('user.register',app()->getLocale())}}"
                    ...
                    <div class="b6">
                        ...
                        <div class="b5">
                            ...
                            <input type="hidden" id="seller_id" value="{{$data['manufacturer']->id}}"
                                   name="merchant_id">
                    ...
                    @if($regions)
                        <div class="b6" id="company_id" style="display: none">
                            <div class="b5">
                                <div class="t3">{{__('phone')}}</div>
                                <input type="text" class="inp" value="{{$data['manufacturer']->phone_number}}" disabled>
                                <input type="hidden" id="seller_id" value="{{$data['manufacturer']->id}}"
                                       name="merchant_id">
                            </div>
                        </div>
...
{{--                            </div>--}}
    ...
    width: 350px;">
                ...
                <div class="b3">
                    ...
                    <div class="b7" style="overflow-y: auto;">
                        ...
                        @foreach($data['products'] as $product)
                            <div class="checkout_item">
                                ...
                                <div class="b8">
                                    <div class="t5">{{$product['product']['name_'.app()->getLocale()]}}</div>
                                    <div class="t6">{{$product['total_quantity']}} {{__('quantity_abbr')}}</div>
                                    <div class="t6">{{number_format($product['total_price'], null, null, ' ')}}</div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="b9">
                        <div class="b6">
                            ...
                            <div class="b5">
                                <input type="hidden" id="cart_price" value="{{$data['total_product_price']}}">
                                <div class="t8"><span><span
                                                id="total_price">{{number_format($data['total_product_price'], null, null, ' ')}}</span> {{__('sum')}}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="t9">{{__('checkout_title')}}
                        <a href="https://cchgeu.ru/upload/staff/ni/%D0%94%D0%BE%D0%B3%D0%BE%D0%B2%D0%BE%D1%80%20%D0%BE%D1%84%D0%B5%D1%80%D1%82%D1%8B.pdf"
                           target="_blank">{{__('checkout_title_1')}}</a>
                    </div>
            ...
            <a href="{{route('user.cart',app()->getLocale())}}" style="display:block;"
    ...
    <script>
                    ...
                    $.ajax({
                        ...
                        success: function (obj) {
                            ...
                            if (obj.price != null) {
                                ...
                                $('#total_price').html(number_format(parseInt(obj.price.price) + parseInt($('#cart_price').val())), null, null, ' ')
                            } else {
                                $('#delivery_payment_d_id').html(" ")
                                $('#delivery_payment_t_id').html(" ")
                            }
        ...
        $('input[name=delivery_type]').on('change', function (event) {
            ...
            if (status == "active") {
                ...
                $('#total_price').html(parseInt($('#cart_price').val()))
                location.reload()
                ...
                $('#total_price').html(number_format(parseInt($('#cart_price').val())), null, null, ' ')
