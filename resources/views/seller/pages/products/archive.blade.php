...
@section('content')
    <div class="main_info seller_panel">
        ...
        <div class="products">
            ...
            @enderror
            @error('success')
            <div class="table_title" style="color: green">{{$message}}</div>
            @enderror
            <form action="{{route('seller.product.archive.search',app()->getLocale())}}" method="post" autocomplete="off">
                @csrf
                ...
                <div class="fltr_btns">
                    ...
                    <a href="{{route('seller.product.archive',app()->getLocale())}}" style="text-decoration: none!important;"
                            ...
                            <tr>
                                ...
                                <th>
                                    ...
                                    <div class="pr">{{__('look_at')}}: <a
                                                href="{{route('user.product_view',['id'=>$product->id,'lang'=>app()->getLocale()])}}">на сайте</a>
                                    </div>

                                </th>
                                <th>{{$product->quantity}}</th>
                                <th>{{$product->price}} сум</th>
                                {{--                                <th class="check"><label class="iosCheck blue">--}}
                                {{--                                        <input type="checkbox" id="update_status_{{$product->id}}"--}}
                                {{--                                               {{$product->is_active ? "checked" : ""}}  onchange="updateProductStatus({{$product->id}},'/seller/product')">--}}
                                {{--                                        <i></i>--}}
                                {{--                                    </label>--}}
                                {{--                                </th>--}}
                                ...
                                <th>
                                    <div class="b1">
                                        ...
                                        <div onclick="updateProductStatus({{$product->id}},'/ajax/product',true)"
                                             style="background: url(/assets/img/icons/reply.png) 0px 0px no-repeat;width: 35px;
    height: 35px;
    cursor: pointer;
    "></div>
    ...
    <input type="hidden" value="{{route('seller.product.archive',app()->getLocale())}}" id="redirect_url">
